    ...
    def forward(
        ...
        **kwargs,
        ...
        if not decode:
            ...
            num_activate_tokens = int(self.sparse * (key_states.shape[2]-self.window_size))
            
            
            if num_activate_tokens > 0 and num_activate_tokens < (key_states.shape[2] - self.window_size) and self.select_kv:
                attn_weights_sum = attn_weights[:, :, -self.window_size:, : -self.window_size].sum(dim = -2)
                attn_cache = F.max_pool1d(attn_weights_sum, kernel_size = self.kernel_size, padding=self.kernel_size//2, stride=1)
                attn_cache = attn_cache.reshape(bsz, self.num_key_value_heads, self.num_key_value_groups, key_states.shape[2] - self.window_size)
                #attn_cache = attn_weights_sum.reshape(bsz, self.num_key_value_heads, self.num_key_value_groups, key_states.shape[2] - self.window_size)
